...
Here I load in data from the "Our World in Data" Project from a csv file called "daily-cases-covid-19.csv" on 23 March 2020. 

```{r load libs and data, message = FALSE, echo = FALSE}

pacman::p_load("readr", "dplyr", "knitr", 
               "here","ggplot2", "assertr", 
               "incidence", "janitor", "forcats")

files <- list(cases = here::here("COVID19/input/daily-cases-covid-19.csv"),
              epicurve1 = here::here("COVID19/output/epicurve_1.png"))

cases_032320 <- as.data.frame(readr::read_delim(files$cases, delim = ",")) %>%
  clean_names() %>%
  transmute(incidence = as.double(daily_new_confirmed_cases_of_covid_19_cases), # too long of a name
            entity = as_factor(entity), 
            code = as_factor(code), 
            year = as.double(year))
cases_032320 <- cases_032320[,order(colnames(cases_032320))]

stopifnot(cases_032320[1, 1] == "AFG")
