...
pacman::p_load("tidyverse", "knitr", "here", "assertr", 
               "EpiCurve", "incidence", "scales", "forcats", "RColorBrewer")

files <- list(cleaned_data = here::here("write/input/full_data_clean.csv"))

data_OWD <- as.data.frame(read_delim(files$cleaned_data, delim="|"))
...
``` {r world_inc, message = FALSE}

world_inc <- data_OWD, loc, date_rec) %>%
  filter (loc == "World")
...
```{r afg_inc, message = FALSE}

afg_inc <- data_OWD %>%
  filter(loc == "Afghanistan")

EpiCurve(afg_inc,
date = "date_rec",
period = "day",
color = "#313695",
xlabel=sprintf("From %s to %s", min(world_inc$date_rec), max(world_inc$date_rec)),
note = "Daily epidemic curve (Afghanistan)")
